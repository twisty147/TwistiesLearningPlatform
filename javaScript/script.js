...
// Define course content for each course
const courseContent = {
  physics: [
...
// Populate course details based on the course name
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to mark a video as watched and trigger completion modal
  function markAsWatched(videoId) {
    ...
    if (listItem) {
      ...
      if (allWatched) {
        ...
        // Show modal congratulating the user and providing physics quiz link
        $('#completionModal').modal('show');

        // Add "Take Quiz" link under topic list
  ...
  // Load the first video automatically when the page loads based on the selected topic
  loadInitialVideo();

  // Function to load the first video automatically based on the selected topic
  function loadInitialVideo() {
    // Parse the query parameters
    const params = new URLSearchParams(window.location.search);
    const courseName = params.get('course');

    // Define the initial video for each course
    const initialVideos = {
      physics: "b1t41Q3xRM8",
    ...
    // Load the initial video based on the selected course
  ...
  // Event listener for course topics
...
// Function to add topic in view to quiz modal

$(document).ready(function () {
  // Function to set the course title in the completion modal
  ...
  // Call the function to set the course title when the document is ready
  setCourseTitle();
...
// Function to handle click event of the "Take Quiz" button
